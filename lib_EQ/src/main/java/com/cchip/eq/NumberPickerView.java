...
public class NumberPickerView extends View {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.NumberPickerView_npv_Orientation) {
                            ...
                            if (mCurrDrawFirstItemY != 0) {//need to adjust
                                ...
                                if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
                                    ...
                                    if (mOrientation == VERTICAL) {
                                        ...
                                        mScroller.startScroll(mCurrDrawGlobalY, 0, mItemHeight + mCurrDrawFirstItemY, 0, duration * 3);
...
//                                    mScroller.startScroll(0, mCurrDrawGlobalY, 0, mItemHeight + mCurrDrawFirstItemY, duration * 3);
                                    ...
                                    if (mOrientation == VERTICAL) {
                                        ...
                                        mScroller.startScroll(mCurrDrawGlobalY, 0, mCurrDrawFirstItemY, 0, duration * 3);
...
//                                    mScroller.startScroll(0, mCurrDrawGlobalY, 0, mCurrDrawFirstItemY, duration * 3);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            mItemDrawHeight = mViewHeight / mShowCount;
            mItemHeight = mViewWidth / mShowCount;
            mViewCenterX = ((float) (mViewHeight + getPaddingTop() - getPaddingBottom())) / 2;
        }
        int defaultValue = 0;
    ...
    protected void onDetachedFromWindow() {
        ...
        if (!mScroller.isFinished()) {
            ...
            if (mOrientation == VERTICAL) {
                ...
                mCurrDrawGlobalY = mScroller.getCurrX();
            }
//            mCurrDrawGlobalY = mScroller.getCurrY();
    ...
    private void scrollByIndexSmoothly(int deltaIndex, boolean needRespond) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            mScroller.startScroll(mCurrDrawGlobalY, 0, dy, 0, duration);
        }
//        mScroller.startScroll(0, mCurrDrawGlobalY, 0, dy, duration);
        if (needRespond) {
    ...
    private void updateDividerAttr() {
        ...
        if (mOrientation == VERTICAL) {
            ...
            dividerY0 = mDividerIndex0 * mViewWidth / mShowCount;
            dividerY1 = mDividerIndex1 * mViewWidth / mShowCount;
        }
        ...
        if (mOrientation == VERTICAL) {
            ...
            if (getPaddingTop() + mDividerMarginL >= mViewHeight - getPaddingBottom() - mDividerMarginR) {
                int surplusMargin = getPaddingTop() + mDividerMarginL + getPaddingBottom() + mDividerMarginR - mViewHeight;
                mDividerMarginL = (int) (mDividerMarginL - (float) surplusMargin * mDividerMarginL / (mDividerMarginL + mDividerMarginR));
                mDividerMarginR = (int) (mDividerMarginR - (float) surplusMargin * mDividerMarginR / (mDividerMarginL + mDividerMarginR));
            }
        }
...
//                    velocityTracker.computeCurrentVelocity(1000);
                    ...
                    if (mOrientation == VERTICAL) {
                        ...
                        velocityY = (int) (velocityTracker.getXVelocity() * mFriction);
                    }
                    if (Math.abs(velocityY) > mMiniVelocityFling) {
                        if (mOrientation == VERTICAL) {
                            ...
                            mScroller.fling(mCurrDrawGlobalY, 0, -velocityY, 0,
                                    limitY(Integer.MIN_VALUE), limitY(Integer.MAX_VALUE), Integer.MIN_VALUE, Integer.MAX_VALUE);
                        }
//                        mScroller.fling(0, mCurrDrawGlobalY, 0, -velocityY,
//                                Integer.MIN_VALUE, Integer.MAX_VALUE, limitY(Integer.MIN_VALUE), limitY(Integer.MAX_VALUE));
...
//            mCurrDrawGlobalY = mScroller.getCurrY();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            drawContentHorizontal(canvas);
            drawLineHorizontal(canvas);
            drawHintHorizontal(canvas);

        }
    }

    private void drawHintHorizontal(Canvas canvas) {
        if (TextUtils.isEmpty(mHintText)) return;
        canvas.drawText(mHintText,
                (dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset, mViewCenterX + (mMaxWidthOfDisplayedValues + mWidthOfHintText) / 2 + mMarginStartOfHint,
                mPaintHint);
    }

    private void drawLineHorizontal(Canvas canvas) {
        if (mShowDivider) {
            canvas.drawLine(dividerY0, getPaddingTop() + mDividerMarginL, dividerY0, mViewHeight - getPaddingBottom() - mDividerMarginR, mPaintDivider);
            canvas.drawLine(dividerY1, getPaddingTop() + mDividerMarginL,
                    dividerY1, mViewHeight - getPaddingBottom() - mDividerMarginR, mPaintDivider);
        }
    }


    private void drawContentHorizontal(Canvas canvas) {
        int index;
        int textColor;
        float textSize;
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            float measureText = mPaintText.measureText("自");
            float fontHeight = TextUtil.getFontHeight(mPaintText);
            if (0 <= index && index < getOneRecycleSize()) {
                canvas.drawText(mDisplayedValues[index + mMinShowIndex].toString(),
                        y + mItemHeight / 2 + textSizeCenterYOffset - fontHeight / 2, mViewCenterX + (fontHeight) / 4, mPaintText);
            } else if (!TextUtils.isEmpty(mEmptyItemHint)) {
                canvas.drawText(mEmptyItemHint,
                        y + mItemHeight / 2 + textSizeCenterYOffset - fontHeight / 2, mViewCenterX + (fontHeight) / 4, mPaintText);
            }
        }
    }
    ...
    private void stopScrolling() {
        ...
        if (mScroller != null) {
            if (!mScroller.isFinished()) {
                if (mOrientation == VERTICAL) {
                    curr = mScroller.getCurrY();
                    mScroller.startScroll(0, mScroller.getCurrY(), 0, 0, 1);
                } else {
                    curr = mScroller.getCurrX();
                    mScroller.startScroll(mScroller.getCurrX(), 0, 0, 0, 1);
                }
//                mScroller.startScroll(0, mScroller.getCurrY(), 0, 0, 1);
                mScroller.abortAnimation();
                postInvalidate();
